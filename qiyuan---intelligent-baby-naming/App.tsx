...
// Updated import to use the new DeepSeek service
...
const App: React.FC = () => {
  ...
  const handleFormSubmit = async (prefs: UserPreferences) => {
    // Basic Client-side Validation for empty card key
    if (!prefs.cardKey) {
       setError("请输入卡密");
    ...
    try {
      ...
      console.error(err);
      // Display the specific error message (e.g., Invalid Card Key)
        ...
        {step === 'results' && results && (
          <div className="animate-fade-in-up">
             ...
             <div ref={resultsRef} data-screenshot-target>
                {/* Only rendering the first result as per 'single result' requirement */}
